...
def main():
    ...
    work_path = f"/data/yaopei/adv_diffusion/" # Replace to yours
    input_path = f"{work_path}dataset/naked_imgs_easy_processed/"

    vec_file = f"{work_path}hf_ring_sd_{args.version}_{args.concept}_vec.pt"
    ...
    with torch.no_grad():
        ...
        org_prompts = get_orig_prompts(num_prompt, concept=args.concept, content="bg")
        get_target(use_imgs, org_prompts, pipe, args, steer_vec, precision_scope, device,
                   target_path, output_path, diff_process, diff_noise_gen, copy.deepcopy(scheduler))

        all_targets = torch.load(target_path)
        if args.version == "p2p":
            negative_prompt_embeds = pipe._encode_prompt([""], device=device,
                                                   num_images_per_prompt=num_images_per_prompt,
                                                   negative_prompt=None,
                                                   do_classifier_free_guidance=True).chunk(3)[2]
    ...
    eval_loader = DataLoader(dataset, batch_size=eval_bs, sampler=eval_sampler, drop_last=True)

    torch_device = pipe.device

    # noise_generator = copy.deepcopy(vae)
    ...
    with torch.enable_grad():
        ...
        for step in range(args.epoch):
            train_loss_ep = 0
            optimizer.zero_grad()
            for iter, (images, prompts, targets, sample_i) in enumerate(train_loader):
                with torch.no_grad():
                    ...
                    if args.version == "p2p":
                        ...
                        # prompt_embedding = torch.cat(prompt_embedding.chunk(3)[:2])
                    else:
                ...
                if args.defense == "noise" or "nadp" in args.attack:
                    #add Gaussian noise with var 0.05
                ...
                if args.attack == "ddpm":
                    ...
                    z_p = pipe.vae.encode(constrained_noisy_imgs).latent_dist.sample()
                    z_p = pipe.vae.decode(z_p, return_dict=False, generator=diff_noise_gen)[0]
                else:
                    z_p, noise_pred, noise_pred_list = diff_process(pipe, scheduler, args, constrained_noisy_imgs, prompt_embedding, negative_prompt_embeds,
                                                   torch_device, diff_noise_gen, args.bs, num_images_per_prompt, defense=args.defense, get_target=False)


                loss = loss_fn(z_p, targets)

                if "sc" in args.attack:
            ...
            if step % 20 == 0:
                ...
                with torch.no_grad():
                        ...
                        with precision_scope():
                            ...
                            if args.defense == "ng":
                                if args.concept == "nudity":
                                    ...
                                    negative_prompt = [""]
                                negative_prompt = negative_prompt * eval_bs
                            else:
                                negative_prompt = None
                            if args.version == "p2p":
                                prompt_embedding = pipe._encode_prompt(prompts, device=device,
                            ...
                            z_p, noise_pred, noise_pred_list = diff_process(pipe, copy.deepcopy(scheduler), args, constrained_noisy_imgs,
                                                           ...
                                                           eval_bs, num_images_per_prompt, defense=args.defense, get_target=False, sld_pipe=sld_pipe)
                    ...
                    eval_loss = eval_loss / (iter+1) * ga_bs
