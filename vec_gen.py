...
def get_nudity_embeddings(model, processor, vec_scale=1):
    ...
    vec_file = "/data/yaopei/steer_vec/clip_steer_vec.pt"
...
def get_nudity_vec(pipe, device, vec_file, num_images_per_prompt = 5, args=None):
    df = pd.read_csv('/data/yaopei/adv_diffusion/dataset/prompts/ring_vectors/Nudity_prompt.csv')
    nudity_text = []
    for _, row in df.iterrows():
        if row.clothes != 'None':
            if row.people == 'None':
                prompt = [f"Make the completely {row.nudity} {row.location}!", f"Let the completely {row.nudity} {row.location}!", f"Put the completely {row.nudity} {row.location}!"]
            else:
                prompt = [f"Make the {row.people} completely {row.nudity}, {row.location}!", f"Let the {row.people} completely {row.nudity}, {row.location}!", f"Put the {row.people} completely {row.nudity}, {row.location}!"]
        else:
            prompt = [f"Make the {row.people} completely {row.nudity}, {row.location}!", f"Let the {row.people} completely {row.nudity}, {row.location}!", f"Put the {row.people} completely {row.nudity}, {row.location}!"]
    ...
    NoNudity_text = []
    df = pd.read_csv('/data/yaopei/adv_diffusion/dataset/prompts/ring_vectors/NoNudity_prompt.csv')
    for _, row in df.iterrows():
        prompt = [f"Make the {row.prompt}", f"Let the {row.prompt}", f"Put the {row.prompt}"]
...
def get_violence_vec(pipe, device, vec_file, num_images_per_prompt = 5, args=None):
    df = pd.read_csv('/data/yaopei/adv_diffusion/dataset/prompts/ring_vectors/Violence_30.csv')

    violence_text = []
    for _, row in df.iterrows():
        prompt = [f"{row.prompt}"]

        embed = pipe._encode_prompt(prompt[0], device=device, num_images_per_prompt=num_images_per_prompt,
...
def main():
    ...
    with torch.no_grad():
        ...
        if args.version == "p2p":
            ...
            prompt_embedding, negative_prompt_embeds = pipe.encode_prompt(text_prompts, device=device,
                                                          num_images_per_prompt=num_images_per_prompt,
                                                          do_classifier_free_guidance=True)
