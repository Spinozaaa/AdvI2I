...
from diffusers import (AutoencoderKL, UNet2DConditionModel, DDPMScheduler, EulerDiscreteScheduler, StableDiffusionImg2ImgPipeline, StableDiffusionInstructPix2PixPipeline,
                       StableDiffusionPipelineSafe, StableDiffusionPipeline, StableDiffusionInpaintPipeline, StableDiffusion3InpaintPipeline, StableDiffusion3Pipeline)
...
def get_orig_prompts(prompt_length):
    ...
    with torch.no_grad(), precision_scope():
        ...
        for img_idx, img in enumerate(all_images):
            ...
            if masks is not None:
                ...
                img_mask = None
            img_batch = img.unsqueeze(0).repeat(batch_size, 1, 1, 1)
            ...
            for start_idx in range(0, num_prompts, batch_size):
                ...
                latents, noise_pred, tar_noise_pred = diff_process(pipe, scheduler, args, img_batch, target_embedding, negative_prompt_embeds, device, generator,
                                                                   num_samples, num_images_per_prompt, defense=args.defense, get_target=True, mask_image=img_mask)
