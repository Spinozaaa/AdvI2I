...
# def diff_process_inpaint(pipe, scheduler, args, imgs, prompt_embedding, negative_prompt_embeds, device, generator,
...
#             latent_model_input = torch.cat([latent_model_input, mask, masked_image_latents], dim=1)            # with torch.no_grad():
#
...
def diff_process(pipe, scheduler, args, imgs, prompt_embedding, negative_prompt_embeds, device, generator,
    ...
    imgs = imgs.to(dtype=pipe.unet.dtype)
    prompt_embedding = prompt_embedding.to(dtype=pipe.unet.dtype)
    ...
    if args.version == "p2p":
        ...
        # masked_image = kwargs["masked_image"]
